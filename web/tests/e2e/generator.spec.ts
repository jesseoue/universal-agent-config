...
test("generates a configuration package", async ({ page }) => {
  ...
  await page.goto("/");
  await expect(page.getByRole("heading", { level: 1 })).toContainText("Build the config");
  ...
  await page.getByRole("button", { name: /OpenRouter Broad model/i }).click();
  await expect(page.getByText("OpenRouter provider routing")).toBeVisible();
